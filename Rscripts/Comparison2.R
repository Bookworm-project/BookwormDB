...
source("Rbindings.R")
con = dbConnect(MySQL(),host='chaucer.fas.harvard.edu',password='fake')
f = dbGetQuery(con,"use arxiv");
dbGetQuery(con,"EXPLAIN catalog")
months = dbGetQuery(con,"SELECT month FROM catalog GROUP BY month")[,1]


p = APIcall(
  list(
    groups = list('CONCAT(words1.word," ",words2.word) as w1'),
    method='counts_query',
    database='arxiv',
    words_collation="Case_Insensitive",
    search_limits = list(
      list(month = as.list(months[61]),archive="physics"),
      list(month = as.list(months[(length(months)-3)]),archive="physics")
     )
    )
)
...
merged= merged[nchar(merged$word)>4,]
dunning = dunning.log(merged)
sort(dunning)[1:100]
sort(-dunning)[1:100]
length(dunning)
dunning[sample(length(dunning),100)]
