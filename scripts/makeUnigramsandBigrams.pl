...
foreach my $filename (@ARGV) {
    ...
    if (-e "$bigramsname") {
        print "$bigramsname already exists... skipping\n";
        next;
    }
...
#This version is designed to work with a list of files for input, and direct them to appropriate places in the file structure.
...
#Keep track of how long it is: break at newlines if more than 64,000 characters, since awk can't handle more than 32,000 columns
        ...
        foreach my $line (@z) {
            ...
            if ($googswitch == 1 || $needs_goog_switch == 0) {
                ...
                #Move period to end of sentence again, in case there's double quotes or a period and parenthesis or something;
                $line =~ s/([\.\?!])(["'\)])/$2$1/g;
                ...
                $stringedLine = $stringedLine . $line;
            }
