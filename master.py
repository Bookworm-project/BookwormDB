...
chunks of equal length at the end: that variable there should probably be set at different levels on different servers.

The goal is to have enough processes to be nice and fast, but not to overload during the 
part that must have items in memory.
'''
...
class bookids_file:
    def __init__(self,targetfiles,mode,auxdata = ""):
        #auxdata is a weird structure so I can pass bookid files or whatever else is needed to the instances. Yuck.
...
class book:
    ...
    def __init__(self,bookid,auxdata=""):
        self.bookid = bookid
