...
logfile.write("sorting tmpfiles... ")
#Formerly I just sorted the keys--but that seems to depend on some local collations, so I think it's safer to sort the files on disk to ensure uniformity between them. This makes it yet slower, though.

for i in range(1,filenum+1):
    subprocess.call(["sort ../texts/wordlist/tmp/" + str(i) + ".txt > ../texts/wordlist/tmp/tmp.txt",shell=True)
    subprocess.call(["mv ../texts/wordlist/tmp/tmp.txt > ../texts/wordlist/tmp/" + str(i) + ".txt",shell=True)
